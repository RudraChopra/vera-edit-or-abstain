...
and preregistered official-code matrices spanning five erasers, five datasets,
and \OfficialReceiptCount audited receipts when the final independent
replication is included. \HeadlineResult
\StressEffectResult
\end{abstract}
...
\IfFileExists{vera_main_results_narrative.tex}{
  ...
  \IfFileExists{vera_independent_stress_results.tex}{}{
    ...
    one-sided seed-blocked raw values were therefore $p=0.125$ and
    Holm-adjusted values were $p=0.5$. We report the effect size because it is
    prespecified and receipted, but do not call the paired comparison
...
the five benchmark datasets a random sample of deployment domains.
The primary paired comparison is reported through its locked seed-blocked
analysis: \SeedBlockedResult. Its sign-flip version relies on paired sign
symmetry; a sign-only sensitivity is also reported. Both tests were fixed before
the relevant seeds were run, and all adjusted values are reported regardless of
outcome. Neither internal audits, exact synthetic coverage, nor a polished
artifact predicts conference acceptance.
