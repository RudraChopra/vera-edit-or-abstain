...
\texttt{ABSTAIN}. This makes VERA orthogonal to erasure methods such as INLP,
LEACE, RLACE, TaCo, SPLINCE, and MANCE++: those methods propose edits, while
VERA certifies whether an edit should be deployed. On official Waterbirds and
Camelyon17 frozen-image benchmarks, VERA produces auditable receipts,
...
VERA addresses this gap by treating representation editing as a certified
...
VERA is a decision layer over candidate erasers, with explicit claim
...
to construct an edit. VERA instead asks whether a candidate edit should be
...
preservation, geometry, or nonlinear concept information. VERA is orthogonal to
...
often through robust losses, reweighting, or group-aware training. VERA has a
...
VERA contribution from robust prediction itself.

The closest statistical neighbors are risk-control and selective-abstention
methods. Risk-controlling prediction sets calibrate set predictors with
finite-sample holdout guarantees \cite{bates2021rcps}. Learn then Test
formulates calibration as multiple testing over candidate predictive
algorithms \cite{angelopoulos2021ltt}, and Pareto Testing controls several
risks over a frontier of candidate configurations \cite{laufer2022pareto}.
Selective classification rejects individual examples to trade coverage for
accuracy \cite{geifman2017selective}. VERA borrows the finite-frontier
risk-control viewpoint, but applies it to representation editing. Its
abstention is edit-level rather than example-level: VERA rejects an entire
representation intervention when validation evidence does not certify the
target/source contract. The novelty claim is therefore not the generic idea of
finite candidate testing; it is the erasure-specific protocol, receipts, and
claim discipline.
...
from an eraser family. VERA evaluates each edited representation
...
target utility and source leakage. VERA's selection rule is fixed before
...
meaningful reduction. If multiple edits are certified, VERA selects the
strongest leakage-reducing edit among them. If no edit is certified, VERA
...
erasers propose a frontier of representation interventions. VERA evaluates the
...
\begin{table}[t]
\centering
\caption{VERA edit-or-abstain protocol. Selection uses validation estimates
...
Let $a_0$ denote the no-edit representation and let
$\mathcal{F}\subseteq\mathcal{A}$ be a finite audited frontier. For each
$a\in \mathcal{F}\cup\{a_0\}$, let $[U^-_a,U^+_a]$ be a simultaneous
validation confidence interval for target utility and let $[L^-_a,L^+_a]$ be a
simultaneous validation confidence interval for source leakage, where larger
$U$ is better and smaller $L$ is better. Assume the intervals cover their
validation quantities for every audited candidate. If VERA accepts only edits
satisfying
\[
...
\begin{proof}
On the simultaneous validation coverage event, no-edit validation utility is at
most $U^+_{a_0}$ and candidate utility is at least $U^-_a$. Thus the validation
target loss is at most $U^+_{a_0}-U^-_a\le\epsilon_U$. Similarly, no-edit
validation leakage is at least $L^-_{a_0}$ and candidate leakage is at most
$L^+_a$, so the certified leakage reduction is at least
$L^-_{a_0}-L^+_a\ge\epsilon_L$. If the certified safe set is empty, the
predeclared decision rule returns \texttt{ABSTAIN}.
\end{proof}

The guarantee is intentionally stated over validation quantities because those
are the quantities the holdout intervals cover. External-shift performance is
reported as a locked diagnostic readout, not as a distribution-free consequence
of validation intervals.
...
If the simultaneous validation intervals used by VERA cover every validation
target and leakage quantity on the audited frontier $\mathcal{F}$ with
probability at least $1-\delta$, then the probability that VERA accepts an
edit violating the validation target-loss or validation leakage-reduction
contract is at most $\delta$.
\end{corollary}

\begin{proof}
By the validation safe-acceptance theorem, on the coverage event every accepted edit
satisfies both validation constraints. A false acceptance can therefore occur
...
This bounded-shift corollary is not used as a distribution-free claim in the
experiments. Camelyon17 is explicitly a shifted hospital benchmark, so VERA's
certificate controls validation quantities; the external split is a locked
diagnostic readout. It is included not as a distribution-free claim about
hospital-shift external performance, but as a measured diagnostic.

\section{Protocol Constants}

All official rows use frozen representations, locked train/validation/external
splits, fresh ridge target/source probes after each edit, and confidence level
$1-\delta=0.95$. For each balanced accuracy, VERA forms per-class recall
intervals using Hoeffding radii
\[
r_c=\sqrt{\frac{\log(2|\mathcal{F}||\mathcal{G}|/\delta)}{2n_c}},
\]
where $\mathcal{G}$ is the set of target classes or target-source groups and
$n_c$ is the validation count for group $c$. Balanced-accuracy intervals are
formed by averaging the covered per-class recall bounds; worst-group intervals
use the most conservative covered group bound.

The full Camelyon17 projection-frontier certificate uses strengths
$\{0,0.1,0.2,0.35,0.5,0.75,1.0\}$, target-loss budget
$\epsilon_U=0.02$, and source-reduction target $\epsilon_L=0.05$. The broader
frontier manifest also records rank candidates $\{1,2,4,8,16,32\}$ and a
default source-reduction budget of $0.02$ for stress sweeps. Every receipt
records split counts, label counts, frontier family, threshold values,
interval construction, and whether external metrics were used in selection. In
the corrected packet, the Camelyon17 certificate explicitly records
...
strength certifies source reduction, so VERA returns \texttt{ABSTAIN}. External
...
Waterbirds, VERA abstains/no-edits because the candidate edit frontier does not
certify a better safe edit; this is a valid VERA decision, not an accuracy win
claim. On Camelyon17, VERA preserves stronger external target and worst-group
...
the official upstream implementation into the VERA harness rather than relying
...
candidate, but they also show why VERA's decision layer is necessary: leakage
...
INLP & Local closed-form row run & Real frozen-feature reference row \\
LEACE & Official code pinned and run & Official frozen-feature reference row \\
...
it does not by itself create reference parity without a matched VERA receipt.
INLP and LEACE now have matched frozen-feature receipts in
\texttt{linear\_eraser\_reference\_report.json}; R-LACE, TaCo, and
SPLINCE/SPLICE remain scoped future reference-parity work.}
\label{tab:reference-boundary}
\end{table}

\section{Reference Linear Eraser Rows}

Because INLP and LEACE are cheap on frozen features, they are no longer left as
proxy-only comparisons. The local packet includes real INLP rows and official
LEACE rows using the pinned \texttt{concept-erasure} implementation. These rows
are not used to claim universal erasure state of the art; they are used to show
the target/source tradeoff that VERA is designed to audit.
...
\bottomrule
\end{tabular}
\caption{Real linear eraser rows from
\texttt{research/artifacts/linear\_eraser\_reference\_report.json}. Lower
source leakage is better, but target and worst-group damage determine whether
an eraser is deployable.}
\label{tab:linear-erasers}
\end{table}

The linear rows strengthen the submission because they address the cheapest
close baselines directly. LEACE sharply reduces Waterbirds leakage but
collapses worst-group accuracy, so an unconditional ``erase more'' objective is
unsafe. On Camelyon17, INLP rank 4 slightly improves target and worst-group
metrics while reducing validation leakage, whereas LEACE reduces leakage more
aggressively with a small target/worst-group cost. VERA's role is to expose
that tradeoff under a registered contract and abstain when the contract is not
certified.

\section{Abstention}

VERA's differentiating behavior is the ability to refuse an edit. The synthetic
...
confidence bound is $0$ for every candidate. VERA therefore abstains: the
...
candidate enters the feasible region, so VERA returns \texttt{ABSTAIN}.}
...
The most important limitation is that VERA is a selection and abstention
protocol, not a universal erasure method. It depends on candidate erasers,
source labels, target labels, and a meaningful validation split. External shift
remains a diagnostic readout unless additional bounded-shift assumptions are
justified. Full-reference MANCE++ evidence is claim-grade for Waterbirds and
Camelyon17, and the packet now includes real INLP plus official LEACE
frozen-feature rows. The manuscript should therefore avoid any statement that
VERA is universally state of the art at nonlinear erasure or clinically
deployable in medical settings.

The strongest reviewer objection is that VERA could be mistaken for an erasure
algorithm and then judged as if it must dominate every modern eraser. The paper
does not make that claim. VERA's contribution is the validation-only
...
VERA reframes representation editing as a decision problem: construct a
...
baselines beyond MANCE++, INLP, and LEACE.
