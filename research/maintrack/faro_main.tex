...
$a \in \mathcal{A}$. If VERA accepts only edits satisfying
...
lower-confidence-bound condition, the selection set is empty, and the VERA
...
output. VERA may be conservative, but under the stated coverage event it does
...
\]
If VERA accepts only edits satisfying
\[
...
empty, VERA returns \texttt{ABSTAIN}.
\end{theorem}
...
and VERA's rule returns \texttt{ABSTAIN}.
\end{proof}
...
bootstrap intervals. VERA does not require these intervals to be tight; wider
...
If the simultaneous validation intervals used by VERA cover every target and
leakage quantity on the audited frontier $\mathcal{F}$ with probability at
least $1-\delta$, then the probability that VERA accepts an edit violating
...
Camelyon17. The manuscript should therefore avoid any statement that VERA is
